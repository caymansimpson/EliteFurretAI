# -*- coding: utf-8 -*-
"""Tests for the memory watchdog in train.py.

The watchdog runs in a daemon thread, samples combined RSS across the
...
internals — by stubbing the RSS sampler.
"""
...
def test_watchdog_disabled_when_threshold_none():
    ...
    from elitefurretai.rl.train import start_memory_watchdog
...
def test_watchdog_fires_event_when_rss_exceeds_threshold():
    """When combined RSS goes above the threshold, the event is set.

    We mock the RSS sampler so the first sample is already above the
    threshold; the daemon thread should set the event and exit promptly.
    """
    from elitefurretai.rl import train as train_mod
    ...
    with patch.object(train_mod, "_sum_process_tree_rss_bytes", return_value=sample):
        thread = train_mod.start_memory_watchdog(
