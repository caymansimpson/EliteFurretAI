...
from elitefurretai.utils.inference_utils import is_grounded, get_showdown_identifier, get_pokemon

"""
This file is a gen7-9 copied damage calculator.
https://github.com/smogon/damage-calc/blob/master/calc/src/mechanics/gen789.ts#L52
"""
...
def calculate_damage(
    ...
    attacker = get_pokemon(attacker_identifier, battle)
    defender = get_pokemon(defender_identifier, battle)

    assert battle.player_role is not None and battle.opponent_role is not None
    assert all(map(lambda x: isinstance(x, int) or isinstance(x, float), attacker.stats.values())), "attacker stats not defined"
    assert all(map(lambda x: isinstance(x, int) or isinstance(x, float), defender.stats.values())), "defender stats not defined"
    ...
    if move.id == "painsplit":
        ...
        if attacker_hp_stat is not None and defender_hp_stat is not None:
            attacker_hp = math.floor(attacker_hp_stat * attacker.current_hp_fraction) if attacker_hp_stat else 0
            defender_hp = math.floor(defender_hp_stat * defender.current_hp_fraction) if defender_hp_stat else 0
    ...
    if defender_ability in ["battlearmor", "shellarmor"]:
        is_critical = False
    elif attacker_ability == "merciless" and defender.status in [Status.PSN, Status.TOX]:
        is_critical = True
    ...
    if move.id == "weatherball":
        ...
        move.id == "terrainpulse" and is_grounded(attacker_identifier, battle)
    ):
    ...
    if move.id == "skydrop" and (
        ...
        return 0, 0
    elif move.id == "steelroller" and not any(map(lambda x: x.is_terrain, battle.fields)):
        return 0, 0
    ...
    defender_identifier = get_showdown_identifier(
        defender,
        battle.player_role if defender in battle.active_pokemon else battle.opponent_role,
    )
    if defender_ability == "wonderguard" and type_effectiveness <= 1:
        ...
        and is_grounded(defender_identifier, battle)
    ):
        return 0, 0
    ...
    attack = calculate_attack(attacker_identifier, defender_identifier, move, battle, is_critical)
    defense = calculate_defense(attacker_identifier, defender_identifier, move, battle, is_critical)

    base_damage = calculate_base_damage(
        attacker_identifier, defender_identifier, base_power, attack, defense, move, move_type, move_target, battle, is_critical
    )
    ...
    final_mods = calculate_final_mods(
        attacker_identifier, defender_identifier, move, battle, type_effectiveness, flags, is_critical
    )
...
def calculate_base_power(
    ...
    attacker_identifier = get_showdown_identifier(
        attacker,
        battle.player_role if attacker in battle.active_pokemon else battle.opponent_role,
    )
    defender_identifier = get_showdown_identifier(
        defender,
        battle.player_role if defender in battle.active_pokemon else battle.opponent_role,
    ...
    if move.id == "payback":
        ...
        base_power = min(
            ...
            60
            + 20
            * sum(map(lambda x: defender.boosts[x], ["atk", "def", "spa", "spd", "spe"])),
        )
        ...
        base_power = base_power * (2 if defender.status in [Status.PSN, Status.TOX] else 1)
        ...
        base_power = base_power * (
            2
            if is_grounded(attacker_identifier, battle)
        ...
        base_power = base_power * (
            2
            if is_grounded(defender_identifier, battle)
            and Field.ELECTRIC_TERRAIN in battle.fields
            else 1
        )
    elif move.id == "psyblade":
        base_power = base_power * (1.5 if Field.ELECTRIC_TERRAIN in battle.fields else 1)
        ...
        if attacker.ability == "prankster" and PokemonType.DARK in defender.types:
            ...
            if attacker.ability == "prankster" and is_grounded(
                defender_identifier, battle
            ):
                base_power = 0
            else:
                base_power = 90
...
def calculate_base_power_mods(
    ...
    if (
        ...
        and is_grounded(attacker_identifier, battle)
        and Field.PSYCHIC_TERRAIN in battle.fields
    ):
        bp_mods.append(6144)
    elif (
    ...
    if is_grounded(attacker_identifier, battle) and (
    ...
    if is_grounded(defender_identifier, battle) and (
        (Field.MISTY_TERRAIN in battle.fields and move.type == PokemonType.DRAGON)
        or (
            Field.GRASSY_TERRAIN in battle.fields and move.id in ["bulldoze", "earthquake"]
        )
    ):
        bp_mods.append(2048)
    ...
    ally_attacking_mon = battle.active_pokemon[1 if attacker == attacking_mons[0] else 0]
    for mon in attacking_mons:
        if (
            mon is not None
    ...
    if ally_attacking_mon and ally_attacking_mon.ability == "powerspot" and not abilities_suppressed:
        bp_mods.append(5325)
    ...
    if defender.ability == "dryskin" and move.type == PokemonType.FIRE and not abilities_suppressed:
        bp_mods.append(5120)
    ...
    if attacker.ability == "supremeoverlord" and num_fainted > 0 and not abilities_suppressed:
...
def calculate_atk_mods(
    attacker_identifier: str, defender_identifier: str, move: Move, battle: DoubleBattle
):
    ...
    attacker_ally = attacker_mons[0] if attacker == attacker_mons[1] else attacker_mons[1]
    ...
    if (
    ...
    elif attacker.ability == "gorillatactics" and move.category == MoveCategory.PHYSICAL:
        atk_mods.append(6144)
    elif (
        (
        ...
        or (
            ...
            and attacker_mons[0]
            and attacker_mons[1]
            and attacker_mons[0].ability in ["plus", "minus"]
            and attacker_mons[1].ability in ["plus", "minus"]
            and not abilities_suppressed
        )
    ):
        atk_mods.append(6144)
    ...
    is_tablets_of_ruin_active = (
        any(map(lambda x: x and x.ability == "tabletsofruin", battle.all_active_pokemons))
        and not abilities_suppressed
    )

    is_vessel_of_ruin_active = (
        any(map(lambda x: x and x.ability == "vesselofruin", battle.all_active_pokemons))
        and not abilities_suppressed
    )
...
def calculate_defense(
    ...
    def_mods = calculate_def_mods(
        attacker_identifier, defender_identifier, move, battle, is_critical, hits_physical
    )
...
def calculate_def_mods(
    attacker_identifier: str,
    defender_identifier: str,
    move: Move,  # TODO: figure out whether this is needed
    battle: DoubleBattle,
    is_critical: bool = False,  # TODO: figure out whether this is needed
    ...
    defender_ally = defender_mons[0] if defender == defender_mons[1] else defender_mons[1]
    ...
    is_swords_of_ruin_active = (
        ...
        or (
            any(
                map(
                    lambda x: x and x.ability == "swordsofruin", battle.all_active_pokemons
                )
            )
    ...
    is_beads_of_ruin_active = (
        ...
        or (
            any(
                map(lambda x: x and x.ability == "beadsofruin", battle.all_active_pokemons)
            )
...
def calculate_base_damage(
    ...
    is_spread = (
        ...
        and defending_mons[0] is not None
        and defending_mons[1] is not None
...
def calculate_final_mods(
    ...
    defending_mons = battle.opponent_active_pokemon if defender in battle.opponent_active_pokemon else battle.active_pokemon  # type: ignore
    defender_ally = (
...
def get_stab_mod(pokemon: Pokemon, move: Move, move_type: Optional[PokemonType]):
    ...
    if tera_type == move_type and tera_type != PokemonType.STELLAR and pokemon.is_terastallized:
        stab_mod += 2048
