...
class SpeedInference:
    ...
    def _get_activations_from_weather_or_terrain(
        ...
        while event is not None and event in ["-activate", "-enditem", "-start", "-boost"]:
            ...
            if event[1] in ["-enditem", "-activate"]:
                ...
                mon = (
                    obs.team[mon_ident]
                    if mon_ident.startswith(str(self._battle.player_role))
    ...
    def check_speed(self, obs: Observation):
        ...
        if self._battle.player_role is None:
            raise ValueError(
        ...
        if any(map(lambda x: x[1] == "start", obs.events)):
            ...
            for event in obs.events:
                if event[-1].replace("ability: ", "") in _PRIORITY_ACTIVATION_ABILITIES:
                    ...
                    if mon_ident is None:
                        raise ValueError(
                            f"We couldnt find the mon_ident and ability for {event}"
                        )

                    mon = (
                        obs.team[mon_ident]
                        if mon_ident.startswith(self._battle.player_role)
                        else obs.opponent_team[mon_ident]
                    )
                    if mon is None:
                        raise ValueError(
            ...
            while event is not None:
                ...
                if (
                    ...
                    # When I happen on an ability, I record it in ability_order
                    ability, mon_ident = parse_ability(event)
                    if mon_ident is None:
                        raise ValueError(
                            f"We couldnt find the mon_ident and ability for {event}"
        ...
        # mon1_scarf = LpVariable(ident + "scarf", cat='Binary')
