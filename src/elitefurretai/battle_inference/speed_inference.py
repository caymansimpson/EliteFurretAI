...
class SpeedInference:
    ...
    def _parse_move(self, events: List[List[str]]) -> List[List[Tuple[str, float]]]:
        ...
        for i, event in enumerate(events):
            ...
            if event[1] == "move":
                ...
                mon_ident = (
                    mon_ident[:2] + mon_ident[3:]
                )  # Removing positional data cuz of skill swap
                mon = get_pokemon(mon_ident, self._battle)
                move = Move(re.sub("[^a-zA-Z]", "", event[3].lower()), self._battle.gen)
                ...
                if (
                    ...
                    "myceliummight" in mon.possible_abilities
                ...
                # Next, check edge cases:
                    ...
                    len(events) > 3
                    and events[i][3] == "Grassy Glide"
                    and Field.GRASSY_TERRAIN in self._battle.fields
                ):
                    priority = 1
                ...
                if multiplier is None or priority is None:
                    ...
                    if last_priority != priority:
                        ...
                        for _, mon in self._battle.team.items():
                            if mon:
                                key = (
                        ...
                        for _, mon in self._battle.opponent_team.items():
                            if mon:
                                key = (
                    ...
                    # were compared against each other
                        ...
                        last_multipliers = (
                            {}
                        )  # We store these mons without position to account for ally switching
                        for _, mon in self._battle.team.items():
                            if mon:
                                key = (
                                    self._battle.player_role
            ...
            # calculations properly. For now, I treat even my own mon's failure to move as a mystery
                ...
                if last_priority is not None:
                    ...
                    # No need to update multipliers or priorities because nothing changed
                    last_moved = mon_ident
            ...
            if key != last_key:
                ...
                for _, mon in self._battle.team.items():
                    if mon:
                        ...
                        last_multipliers[ident] = self._generate_multiplier(ident)
                ...
                # Need to update; if a mon faints because of a residual, we should remove it
                for _, mon in self._battle.team.items():
                    if mon:
                        ident = (
                            self._battle.player_role
    ...
    def _parse_switch(self, events: List[List[str]]) -> List[List[Tuple[str, float]]]:
        ...
        # We need to store these to understand what was there before the switch. Note that we use the internal
        # field, which is a Dict of mon_ident -> mon
        ...
        if isinstance(self._battle, DoubleBattle):
            ...
            # I have to store the info of the mons before they switch, lest temporary effects get washed out after they
    ...
    def _solve_speeds(self, orders: List[List[Tuple[str, float]]], obs, segments):
        ...
        for key, mon in self._battle.team.items():
            if (
                mon.stats and mon.stats["spe"]
            ):  # TODO: this isnt triggering because I dont have proper stats storage
                variables[key] = mon.stats["spe"]

            # TODO: remove once we fix stats; this is to help the program run; otherwise this will be None
            else:
