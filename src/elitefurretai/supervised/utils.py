...
def evaluate(
    ...
    for batch in _eval_iterator:
        ...
        if (
            ...
            if has_valid.any():
                ...
                final_win = wins[batch_idx, last_idx]  # [batch]
                binary_outcome = (final_win > 0).float().unsqueeze(1).expand_as(wins)
                pred_prob = (win_logits + 1.0) / 2.0  # tanh outputs → [0, 1]
                valid_mask = masks.bool() & has_valid.unsqueeze(1)
                brier_sq = (pred_prob - binary_outcome) ** 2
                metrics["brier_sum"] += brier_sq[valid_mask].sum().item()
                metrics["brier_count"] += int(valid_mask.sum().item())
    ...
    if steps > 0:
        ...
        if metrics["brier_count"] > 0:
            metrics["brier_score"] = metrics["brier_sum"] / metrics["brier_count"]
...
def analyze(
    ...
    verbose: bool = True,
    state_input_dim: Optional[int] = None,
...
def create_empty_metrics() -> Dict[str, Any]:
    ...
    return {
        ...
        "win_preds": [],
        "win_targets": [],
        # Brier score: ((pred_prob - actual_binary_outcome) ** 2), where the binary
        # outcome is the trajectory-final win label broadcast to all valid steps
