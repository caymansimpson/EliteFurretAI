...
from elitefurretai.rl2.team_manager import TeamPool
...
def worker_loop(opponent_pool: OpponentPool, traj_queue: queue.Queue, num_players: int, 
                worker_id: int, port_offset: int, team_pool: Optional[TeamPool] = None):
    """
    ...
    Args:
        ...
        team_pool: Optional TeamPool for random team sampling
    """
    loop = asyncio.new_event_loop()
    asyncio.set_event_loop(loop)
    
    server_port = 8000 + port_offset
    server_config = ServerConfiguration(f"localhost:{server_port}", None)
    
    players = []
    for i in range(num_players):
        # Sample random team if pool provided
        team = team_pool.sample_team() if team_pool else None
        
        player = BatchInferencePlayer(
            model=opponent_pool.main_model,
            device=opponent_pool.device,
            batch_size=16,
            player_configuration=PlayerConfiguration(f"Worker_{worker_id}_{i}", None),
            server_configuration=server_config,
            trajectory_queue=traj_queue,
            battle_format="gen9vgc2023regulationc",
            team=team  # Use random team
        )
    ...
    async def run_battles():
        while True:
            ...
            opponents = []
            for i in range(num_players):
                opp_config = PlayerConfiguration(f"Opponent_{worker_id}_{i}", None)
                opponent_team = team_pool.sample_team() if team_pool else None
...
def collate_trajectories(trajectories, device):
    ...
    for i in range(batch_size):
        l = len(processed_advantages[i])
        advantages[i, :l] = processed_advantages[i]
        returns[i, :l] = processed_returns[i]
...
def save_checkpoint(model: RNaDAgent, optimizer, step: int, config: RNaDConfig, 
                   curriculum: Dict, save_dir: str = "data/models"):
...
def train_exploiter_subprocess(victim_checkpoint: str, config: RNaDConfig):
    ...
    print(f"\n{'='*60}")
    print(f"EXPLOITER TRAINING TRIGGERED")
    ...
    cmd = [
        sys.executable,
        "src/elitefurretai/rl2/train_exploiter.py",
        "--victim", victim_checkpoint,
        "--steps", str(config.exploiter_train_steps),
        "--eval-games", str(config.exploiter_eval_games),
        "--threshold", str(config.exploiter_win_threshold),
        "--output-dir", str(config.exploiter_output_dir)
    ]
    
    if config.exploiter_team_pool_path:
        cmd.extend(["--team-pool", config.exploiter_team_pool_path])
...
def main():
    ...
    # Load team pool if provided
    team_pool = None
    if config.team_pool_path:
        try:
            team_pool = TeamPool(config.team_pool_path)
            print(f"Loaded {len(team_pool)} teams from {config.team_pool_path}")
        except Exception as e:
    ...
    for i in range(config.num_workers):
        ...
        t = threading.Thread(
            target=worker_loop, 
            args=(opponent_pool, traj_queue, config.players_per_worker, i, port_offset, team_pool),
            daemon=True
        )
        t.start()
        threads.append(t)
    ...
    print(f"Workers distributed across ports 8000-8003")
    ...
    try:
        while updates < config.max_updates:
            ...
            if len(trajectories) >= config.train_batch_size:
                ...
                if config.train_exploiters and (updates - last_exploiter_check >= config.exploiter_check_interval):
