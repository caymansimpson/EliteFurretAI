...
└─────────────────────────────────────────────────────────────────┘
    |
    | ff_out_early: (batch, seq_len, 512)
    |───────────────────────────────┬─────────────────────────────┐
    |                               |                             |
    |                               v                             |
    |                    ┌──────────────────────────┐             │
...
out: (batch, seq_len, 256)
    ...
    v                             v                             v
┌──────────────────┐      ┌─────────────────┐         ┌─────────────────┐
│ TURN ACTION HEAD │      │ WIN HEAD        │         │ TEAMPREVIEW HEAD│
│ Linear(256 →     │      │ Linear(256→128) │         │ (from ff_out_   │
│   2025 actions)  │      │ LayerNorm       │         │  early branch)  │
└──────────────────┘      │ Linear(128→1)   │         │ → 90 actions    │
(batch, seq_len, 2025)    │ Tanh            │         └─────────────────┘
                          └─────────────────┘          (batch, seq_len, 90)
                           (batch, seq_len)
                           values ∈ [-1, 1]
...
class FlexibleThreeHeadedModel(torch.nn.Module):
    def __init__(
        ...
        teampreview_head_layers: list = None,
        ...
        if teampreview_attention_heads > 0:
            self.teampreview_attn = torch.nn.MultiheadAttention(
                teampreview_output_size, teampreview_attention_heads, batch_first=True, dropout=teampreview_head_dropout
            )
            self.teampreview_ln = torch.nn.LayerNorm(teampreview_output_size)
        else:
        ...
        if use_grouped_encoder:
            self.skip_proj: Optional[torch.nn.Linear] = torch.nn.Linear(self.hidden_size, lstm_hidden_size * 2)
    ...
    def forward(
        ...
        # Output heads
        turn_action_logits = self.turn_action_head(out)  # (batch, seq_len, 2025)
...
class ThreeHeadedModelWrapper(torch.nn.Module):
    """
    Wrapper to make FlexibleThreeHeadedModel compatible with existing evaluate/analyze utilities.
    Merges turn and teampreview predictions into a single action head for evaluation.
    """
    def __init__(self, model: FlexibleThreeHeadedModel, teampreview_idx: int):
        super().__init__()
        self.model = model
        self.teampreview_idx = teampreview_idx
    
    def forward(self, x: torch.Tensor, mask: Optional[torch.Tensor] = None):
        """
        Returns merged action logits and win logits for compatibility with evaluate/analyze.
        
        For teampreview samples: uses teampreview_logits (90-dim) padded to 2025
        For turn samples: uses turn_action_logits (2025-dim)
        """
        turn_logits, tp_logits, win_logits = self.model(x, mask)
        
        # Determine which samples are teampreview
        teampreview_mask = x[:, :, self.teampreview_idx] == 1  # (batch, seq)
        
        # Create merged action logits
        merged_logits = turn_logits.clone()  # Start with turn logits (batch, seq, 2025)
        
        # For teampreview samples, zero out turn actions and fill in teampreview actions
        # teampreview actions are in positions [0:90]
        merged_logits[teampreview_mask] = float("-inf")  # Zero out all
        merged_logits[teampreview_mask, :MDBO.teampreview_space()] = tp_logits[teampreview_mask]
        
        return merged_logits, win_logits
    ...
    for batch in dataloader:
        ...
        with autocast(config['device']):
            ...
            if teampreview_valid_mask.any():
                ...
                teampreview_loss = topk_cross_entropy_loss(flat_tp_logits, flat_tp_actions, k=3)
        ...
        del states, actions, action_masks, wins, masks
...
def initialize(config):
    # Wandb defaults
    wandb.init(
        project="elitefurretai-three-headed",
        config=config,
...
def main(train_path, test_path, val_path):
    ...
    config: Dict[str, Any] = {
        ...
        "worker_batch_size": 32,  # Reduced for larger model

        # Basic Training Params
        "learning_rate": 1e-4, # 5e-5,  # Lower for larger model
        "optimizer": "AdamW",  # Better for large models
        "num_epochs": 20,

        # Regularization
        "dropout": 0, #  0.1,  # Increased for regularization
        "weight_decay": 1e-5,  # Increased from 0
        "max_grad_norm": 1.0, # 2.0,  # Increased from 1.0
        ...
        # Data Weighting (deprecated - now using separate heads)
        "weights": False,
        "teampreview_weight": 1.0,
        "force_switch_weight": 1.0,
        "turn_weight": 1.0,
        "action_loss_weight": 1,  # Kept for backward compat

        # Architecture - Backbone
        "early_layers": [2048, 2048, 1024, 512], #[4096, 4096, 2048, 2048, 1024],
        "late_layers": [], # [2048, 1024, 512],
        "lstm_layers": 2, #3,
        "lstm_hidden_size": 512, # 1024,

        # Architecture - Grouped Encoder
        "use_grouped_encoder": True,
        "grouped_encoder_hidden_dim": 128, # 512,
        "grouped_encoder_aggregated_dim": 1024, # 4096,
    ...
    for epoch in range(config["num_epochs"]):
        ...
        eval_model = ThreeHeadedModelWrapper(model, config["teampreview_idx"])
        metrics = evaluate(
            eval_model,
            test_loader,
            config["device"],
        ...
        test_loss = (
            ...
            metrics["top3_loss"] * (config["turn_loss_weight"] + config["teampreview_loss_weight"]) / 2
        )
