...
def main(train_path, test_path, val_path):
    ...
    for epoch in range(config["num_epochs"]):
        ...
        print(
            f"=> Test Loss:         {(metrics['win_loss'] * config['win_weight'] + metrics['action_loss'] * config['action_weight']):.4f}"
        )
        print(f"=> Test Win Acc:      {metrics['win_acc'] * 100:.3f}%")
        print(f"=> Test Win Loss:     {metrics['win_loss']:.4f}")
        print(f"=> Test Action Loss:  {metrics['action_loss']:.4f}")
        print(
        ...
        wandb.log(
            {
                ...
                "test_loss": metrics["win_loss"] * config["win_weight"]
                + metrics["action_loss"] * config["action_weight"],
                "test_action_loss": metrics["action_loss"],
                "test_win_loss": metrics["win_loss"],
                "test_win_acc": metrics["win_acc"],
        ...
        scheduler.step(float(metrics["win_loss"] + metrics["action_loss"]))
    ...
    print(
        f"=> Val Loss:          {(metrics['win_loss'] * config['win_weight'] + metrics['action_loss'] * config['action_weight']):.3f}"
    )
    print(f"=> Val Win Loss:      {metrics['win_loss']:.3f}")
    print(f"=> Val Action Loss:   {metrics['action_loss']:.3f}")
    print(f"=> Val Win Acc:       {metrics['win_acc']:.3f}")
    print(
        f"=> Val Action Acc:    {(metrics['action_top1'] * 100):.3f}% (Top-1), {(metrics['action_top3'] * 100):.3f}% (Top-3) {(metrics['action_top5'] * 100):.3f}% (Top-5)"
    )
    wandb.log(
        {
            "val_loss": metrics["win_loss"] * config["win_weight"]
            + metrics["action_loss"] * config["action_weight"],
            "val_win_loss": metrics["win_loss"],
            "val_win_acc": metrics["win_acc"],
            "val_action_loss": metrics["action_loss"],
            "val_action_top1": metrics["action_top1"],
            "val_action_top3": metrics["action_top3"],
            "val_action_top5": metrics["action_top5"],
        }
    )
    analyze(model, val_loader, embedder.feature_names, config["device"])
