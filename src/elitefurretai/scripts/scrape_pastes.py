...
def scrape_html_page():
    ...
    while not queue.empty():
        ...
        try:
            ...
            with lock:
                tried += 1
                sys.stdout.write(f"\033[2K\rRead {tried} pokepastes so far... starting on {url}")
                sys.stdout.flush()       
            ...
            for text in [pre.get_text(strip=True) for pre in soup.find_all('pre')]:
                ...
                mon = mon.replace("U-turn", "Uturn").replace("Will-O-Wisp", "WillOWisp").replace("Freeze-Dry", "FreezeDry")
                mon = mon.replace("Self-Destruct", "SelfDestruct").replace("Double-Edge", "DoubleEdge")
                mon = mon.replace("Baby-Doll Eyes", "BabyDollEyes").replace("X-Scissor", "XScissor").replace("Soft-Boiled", "SoftBoiled")
                ...
                mon = split[0] + "\nAbility:" + re.sub(r"\s?+- ?", "\n- ", split[1])
                mon = mon.replace("Uturn", "U-turn").replace("WillOWisp", "Will-O-Wisp").replace("FreezeDry", "Freeze-Dry")
                mon = mon.replace("SelfDestruct", "Self-Destruct").replace("DoubleEdge", "Double-Edge")
                mon = mon.replace("BabyDollEyes", "Baby-Doll Eyes").replace("XScissor", "X-Scissor").replace("SoftBoiled", "Soft-Boiled")
...
def main():
    ...
    for r in results:
        ...
        if "untitled" in team_name.lower() or "copy" in team_name.lower() or len(team_name.strip()) <= 3:
            errors += 1
    ...
    print(f"Finished! Went through {tried} links, with {tried - errors} successes in {int(duration)} secs!")
    print("\n============ Errors ============")
    if len(error_msgs["bad_name"]) > 0:
        print(f"Not saving teams that don't have legitimate team name: [{', '.join(sorted(error_msgs["bad_name"]))}]\n")
    if len(error_msgs["zoroark"]) > 0:
        print(f"I refuse to parse this team because I found zoroark: [{', '.join(sorted(error_msgs["zoroark"]))}]\n")
    if len(error_msgs["missing_EVs"]) > 0:
        print(f"Cant parse team because it has a mon without EVs: [{', '.join(sorted(error_msgs["missing_EVs"]))}]\n")
