...
The script also standardizes VGC teams to ensure all Pokemon have Level: 50,
which is necessary for accurate stat parsing from the teambuilder.
"""
...
def standardize_vgc_levels(team_string: str) -> str:
    """
    Standardize VGC team text to ensure all Pokemon have Level: 50.
    This is necessary for accurate stat parsing from the teambuilder.

    Args:
        team_string: PokePaste format team text

    Returns:
        Standardized team text with Level: 50 for all Pokemon
    """
    # Replace any existing level or add Level: 50 after Ability line
    team_string = re.sub(
        r"(Ability:.*\n)(Level:\s[0-9]+(\s+)?\n)?", r"\1Level: 50\n", team_string
    )
    return team_string
    ...
    while not queue.empty():
        ...
        try:
            ...
            for text in [pre.get_text(strip=True) for pre in soup.find_all("pre")]:
                ...
                # Standardize VGC levels if this is a VGC format
                if "vgc" in frmt.lower():
                    mon = standardize_vgc_levels(mon)
